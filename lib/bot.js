...
import { formatFromIRCToDiscord, toPlainText } from './formatting'

// Usernames need to be between 2 and 32 characters for webhooks:
const USERNAME_MIN_LENGTH = 2
const USERNAME_MAX_LENGTH = 32
const maxClientChatLength = 233
...
class Bot {
  ...
  parseText(message) {
    ...
    return text
      ...
      .replace(/<@&(\d+)>/g, (_match, roleId) => {
        const role = message.guild.roles.cache.get(roleId)
        if (role) return `@${role.name}`
        return '@deleted-role'
      })
      .replace(/<a?(:\w+:)\d+>/g, (_match, emoteName) => emoteName)
  }
  ...
  sendToIRC(message) {
    ...
    if (this.ircNickColor) {
      const colorIndex =
        (nickname.charCodeAt(0) + nickname.length) % this.ircNickColors.length
    ...
    if (this.isCommandMessage(text)) {
      ...
      if (text !== '') {
        ...
        // this is a hack to get around my lack of c knowledge
        // ideally we'd truncate in hercules rather than here, but whatever
        const truncateLength = maxClientChatLength - `[ #dc ] `.length

        text = _.truncate(text, { length: truncateLength })
  ...
  sendToDiscord(author, channel, text) {
    ...
    const withMentions = withFormat
      ...
      .replace(/^([^@\s:,]+)[:,]|@([^\s]+)/g, (match, startRef, atRef) => {
        ...
        const nickUser = guild.members.cache.find(
          x => x.nickname && Bot.caseComp(x.nickname, reference)
        )
        ...
        const checkMatch = function (matchString, matchValue) {
          ...
          if (
            (matchString.length > matchLength &&
              Bot.caseStartsWith(reference, matchString)) ||
            (matchString.length === matchLength &&
              !caseMatched &&
